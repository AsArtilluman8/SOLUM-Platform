...
```text
P55 — Sky / Weather Core Pack
```

Main goal:

```text
Add first SOLUM-owned sky/weather architecture and lightweight visual foundation while keeping Ultra Dynamic Sky local-reference-only.
```
...
- Sky / IBL has P51 Environment API controls, P52 asset status, and P55 Sky/Weather Core controls. Real HDRI/star/cloud textures remain planned unless generated by SOLUM or verified CC0/permissive.
...
- P55 requires static checks first and one full build maximum unless a compile fix is needed.
