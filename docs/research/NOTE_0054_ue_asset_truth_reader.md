...
P59 adds a broader 341-package corpus and removes that volume-texture limitation without flattening. All 38 supplied texture sources are now verified: 32 Texture2D PNG files, three VolumeTexture NPY arrays, one TextureCube Radiance HDR and two floating-point Texture2D NPY arrays. Trailer-backed and legacy bulk layouts, uncompressed and Oodle `FCompressedBuffer`, legacy chunked zlib, G8/BGRA8/RGBA8/BGRE8 UEDELTA, integer and half/float source formats are kept as distinct bounded paths. When `TextureSource.Id` is hash-derived, the chosen inverse must reproduce its BLAKE3 digest; serialized dimensions, byte length, channel extrema and output-container structure are also checked.

Material-family coverage grows to 174/174 verified contracts: 112 serialized Material/Function graphs, 59 Material Instances and three Material Parameter Collections. Instance parents, scalar/vector/texture/font/runtime-virtual-texture overrides and static parameter sets retain object identity and native-byte provenance. Parameter collections validate their declared scalar/vector defaults and state GUIDs. This is still a material contract, not shader or HLSL reconstruction.

P59 Niagara coverage includes 20/20 exact script summaries and two Niagara Parameter Collections with 106 default parameter spans matched byte-for-byte to their typed variables. Script usage, usage GUID, rapid-iteration parameters, executable/VM bytecode hashes, compile hashes, data interfaces, object bindings and function-call dependencies are emitted together with exact graph links. Native regions not represented by an implemented serializer keep `RAW_VERIFIED`, so a verified sub-contract does not overstate the entire asset.

The same archive verifies eight curve assets and 23 Blueprint-family packages: 18 contain non-empty graphs and five are correctly identified as data-only. The Kismet bytecode path remains exact. Nine of nine StaticMesh fixtures export valid GLB with 5,715 total indices; seven use package trailers and two exercise the UE5.0/5.1 disk-backed editor-bulk path. Four embedded mono PCM16 SoundWave sources export as exact WAV and a MetaSound contract resolves 30 referenced audio assets.

## Known limits
Verified GLB currently covers UE5 editor-domain `FMeshDescription` stored locally in a v0-v2 package trailer or the proven UE5.0/5.1 disk-backed editor-bulk layout. The implemented legacy FByteBulkData reader covers its exact metadata widths and locally addressable payloads, but cooked `FStaticMeshLODResources`, BCn platform texture mips, chunked/streaming audio, IoStore/Zen and virtualized/remote payloads remain separate versioned paths. UEDELTA is restricted to the verified one-mip G8/BGRA8/RGBA8/BGRE8 layouts. Material, Blueprint, MetaSound and Niagara executable semantics are represented as data/graph/bytecode contracts, not invented HLSL or C++. The available archives contain no matching cooked `.uexp/.ubulk/.uptnl` or `.utoc/.ucas` set, so those paths still require legal companion fixtures for proof.
