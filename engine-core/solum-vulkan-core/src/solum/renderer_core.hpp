...
struct RendererCore {
    ...
    void syncLightingModelState() {
        ...
        model.iblMode = "directional_sky_ground_ibl";
        model.environmentIblStatus = "ok_foundation";
        model.environmentIblMode = "directional_sky_ground_ibl";
        model.environmentSourceStatus = "ok_procedural_no_external_texture";
        model.environmentSourceType = "directional_sky_ground_shader_model";
        model.environmentSkyColorStatus = "ok_preset_uniform";
        model.environmentGroundColorStatus = "ok_preset_uniform";
        model.environmentHorizonStatus = "ok_directional_horizon_blend";
        ...
        model.reflectionFoundationStatus = "p18_environment_ibl_foundation";
        model.reflectionMode = "directional_sky_ground_ibl";
        model.environmentReflectionStatus = "p18_environment_directional_source_no_texture_cubemap";
        model.environmentReflectionMode = "reflection_direction_sky_ground_ibl";
        model.environmentSource = "directional_sky_ground_shader_model";
        model.reflectionColorStatus = "ok_environment_preset_horizon_gradient";
        model.reflectionRoughnessResponseStatus = "ok_roughness_blurs_reduces_reflection";
        model.metallicReflectionStatus = "ok_metal_tinted_environment_guarded";
        ...
        model.materialResponseStatus = "p18_environment_ibl_foundation";
    ...
    bool setLightingControls(int lightPreset, float sunIntensity, float ambientIntensity, int activeDebugView, int toneMappingMode, float exposureValue, float ambientFloor, int brightnessPreset, float specularBoost, float reflectionIntensity, float contactShadowIntensity, int calibrationPreset, float calibrationStrength, float glossSliderValue, float paintGlossSliderValue, float environmentIntensity, int environmentPreset, float horizonStrength, int selectedSlot, float slotMetallic, float slotRoughness, float slotNormalScale, float slotAo, float slotGloss, float slotCoat) {
        ...
        syncDiagnostics();
        diagnostics.write(ok ? "valid" : diagnostics.status, ok ? "Scene17 runtime material workflow updated selected slot uniforms only." : diagnostics.reason);
        updateReadyStatus();
        return ok;
    }
    ...
    bool uploadModelFirstPrimitive(
        ...
        syncDiagnostics();
        diagnostics.write("valid", "Scene17 Runtime Material Workflow Lab uploaded and drew first primitive.");
        updateReadyStatus();
        return true;
    }
    ...
    bool uploadModelMultiPrimitive(
        ...
        syncDiagnostics();
        diagnostics.write("valid", "Scene17 Runtime Material Workflow Lab uploaded and drew supported primitives.");
        updateReadyStatus();
        return true;
    }
    ...
    bool init(ANativeWindow* window, int width, int height, const std::string& root) {
        ...
        syncDiagnostics();
        diagnostics.write("valid", "Scene17 Runtime Material Workflow Lab initialized with cube fallback while waiting for active model upload.");
        updateReadyStatus();
        return true;
    }
};
