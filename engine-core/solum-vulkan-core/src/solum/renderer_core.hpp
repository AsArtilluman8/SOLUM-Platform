...
struct RendererCore {
    ...
    bool isGlassRoute(const MaterialSlotState& slot) const {
        return slot.materialRoute == MaterialRoute::GlassBlend || slot.materialRoute == MaterialRoute::GlassSemanticOpaque;
    }

    bool isSelectedSlot(int materialSlot) const {
    ...
    void syncGlassState() {
        ...
        rebuildRenderQueues();
        const bool queueActive = material.glassEnabled != 0 && !glassQueue.empty();
        const bool active = material.glassEnabled != 0 || selectedGlass || activeMaterialPreset == 6 || routedGlassSlot >= 0 || queueActive;
        const bool transparentActive = queueActive || (routedGlassSlot >= 0 && transparentAllowed);
        ...
        model.glassMaterialStatus = queueActive ? "ok_transparent_glass_materials_detected" : (active ? "ok_p26_polished_single_pass" : "available");
        model.glassMode = queueActive ? "transparent_glass_queue_pipeline" : (transparentActive ? "transparent_v1_role_routed_no_refraction" : "single_pass_fake_transparency_no_refraction");
    ...
    bool setLightingControls(int lightPreset, float sunIntensity, float ambientIntensity, int activeDebugView, int toneMappingMode, float exposureValue, float ambientFloor, int brightnessPreset, float specularBoost, float reflectionIntensity, float contactShadowIntensity, int calibrationPreset, float calibrationStrength, float glossSliderValue, float paintGlossSliderValue, float clearcoatIntensity, float clearcoatRoughness, float environmentIntensity, int environmentPreset, float horizonStrength, float reflectionContrast, float reflectionSaturation, float motionReflectionScale, float motionClearcoatScale, int selectedSlot, float slotMetallic, float slotRoughness, float slotNormalScale, float slotAo, float slotGloss, float slotCoat, float alphaCutoffValue, float emissiveIntensity, int materialPreset, int glassEnabled, float glassOpacity, float glassEdge, float glassRoughness, int glassTintPreset, int glassRenderMode, int glassType, int manualGlassSlotOverride) {
        ...
        material.activeDebugView = ((activeDebugView % 75) + 75) % 75;
            ...
            auto drawQueue = [&](const std::vector<PrimitiveDrawRange>& queue, PipelineBundle& pipeline, bool glassPass) {
                ...
                if (range.materialSlot >= 0 && (size_t)range.materialSlot < modelMaterialSlots.size()) {
                    ...
                    if (slot.hasClearcoat && slot.clearcoatFactor > 0.001f) {
                        pc.material.clearcoatIntensity = slot.clearcoatFactor;
                        pc.material.clearcoatRoughness = clampFloat(slot.clearcoatRoughnessFactor, 0.04f, 1.0f);
                    } else if (clearcoatAllowed) {
                        pc.material.clearcoatIntensity = material.clearcoatIntensity;
                        pc.material.clearcoatRoughness = material.clearcoatRoughness;
                    }
