...
if [ ! -d "$(dirname "$LOG_ROOT")" ]; then LOG_ROOT="/storage/emulated/0/Download/SOLUMCreative"; fi
...
CXX="$(find_android_clang)" || { echo "No executable Android/Termux clang++ found" | tee "$LOG"; exit 1; }
...
if [ -z "$ANDROID_STUB_LIB_DIR" ]; then echo "No Android NDK Vulkan stub lib dir found. Refusing to link against Termux libvulkan.so.1." | tee "$LOG"; exit 1; fi
{
  ...
  if ! "$CXX" -std=c++17 -O2 -fPIC -shared "$SRC" -o "$OUT" -I"$ROOT/engine-core/solum-vulkan-core/src" -L"$ANDROID_STUB_LIB_DIR" -landroid -lvulkan -llog -static-libstdc++; then
    echo "static-libstdc++ build failed; retrying without it"; "$CXX" -std=c++17 -O2 -fPIC -shared "$SRC" -o "$OUT" -I"$ROOT/engine-core/solum-vulkan-core/src" -L"$ANDROID_STUB_LIB_DIR" -landroid -lvulkan -llog
  fi
  set +x; echo; copy_libcxx_shared_if_needed; echo; echo "Runtime dynamic dependencies:";
  if command -v readelf >/dev/null 2>&1; then readelf -d "$OUT" | grep NEEDED || true; elif command -v llvm-readelf >/dev/null 2>&1; then llvm-readelf -d "$OUT" | grep NEEDED || true; else echo "readelf not available"; fi
  if command -v readelf >/dev/null 2>&1 && readelf -d "$OUT" | grep -q 'libvulkan.so.1'; then echo "ERROR: libsolum_engine.so still depends on libvulkan.so.1. Android needs libvulkan.so."; exit 1; fi
  echo; echo "Packaged jniLibs:"; ls -la "$OUT_DIR"
} > "$LOG" 2>&1
