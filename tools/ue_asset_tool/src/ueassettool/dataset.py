...
def _classify_contract(contract: dict[str, Any]) -> str:
    ...
    return {
        ...
        "ueassettool.niagara-contract/v2": "niagara",
...
def _contract_metrics(contract: dict[str, Any]) -> dict[str, int]:
    ...
    is_material = contract.get("schema") == "ueassettool.material-contract/v1"
    metrics = {
        ...
        "material_parameters": len(contract.get("parameters", [])) if is_material else 0,
        "material_functions": sum(root.get("class") in ("MaterialFunction", "MaterialFunctionInstance") for root in contract.get("roots", [])),
        "mic": len(contract.get("material_instances", [])) if is_material else 0,
        "mpc": len(contract.get("parameter_collections", [])) if is_material else 0,
        "scalar_parameters": 0,
        "vector_parameters": 0,
...
def _try_media(source: Path, output_base: Path, kind: str) -> tuple[dict[str, Any] | None, list[str]]:
    suffixes = (".png", ".jpg", ".hdr", ".npy") if kind == "texture" else (".wav", ".ogg")
    errors = []
...
def build_dataset(archive: Path, dataset: Path, repo: Path) -> dict[str, Any]:
    ...
    checks = [
        ...
        _run_check(
            [sys.executable, "-m", "unittest", "discover", "-s", "tools/ue_asset_tool/tests", "-v"],
            repo,
            test_log,
            "unit_and_regression_tests",
            extra_environment={
                "UEASSET_P59_ARCHIVE": str(archive),
                "UEASSET_P59_EXTRACTED": str(input_root / "assets"),
            },
        ),
...
- Textures: exact editor TextureSource for Texture2D, TextureCube and VolumeTexture; semantic outputs are PNG, HDR or NPY. Cooked platform texture payloads remain unsupported.
- Models: exact editor FMeshDescription from package trailers or verified disk-backed FEditorBulkData; cooked LOD resources remain unsupported.
...
All blockers are enumerated in `errors.json` and `EXTRACTION_GATE.json`. Missing sidecars are recorded per package. Oodle-compressed payloads require an explicitly supplied licensed helper; this run used one when needed. Further exact decoding of cooked-only layouts requires version-matched complete cooked package sets and their owning `.uexp`/`.ubulk`/`.uptnl` sidecars where declared.
