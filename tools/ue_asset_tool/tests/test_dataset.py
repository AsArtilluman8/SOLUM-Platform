...
from ueassettool.dataset import (
    SCHEMA_VERSION,
    TERMINAL_STATUSES,
    _contract_metrics,
    sha256_file,
...
class CanonicalDatasetTests(unittest.TestCase):
    def test_niagara_parameter_collection_is_not_counted_as_mpc(self) -> None:
        metrics = _contract_metrics({
            "schema": "ueassettool.niagara-contract/v2",
            "parameter_collections": [{"object": "/Game/NPC"}],
        })
        self.assertEqual(metrics["mpc"], 0)
